...
class Slowmode(commands.Cog):
    ...
    @commands.command(aliases=["slow"], description="Changes the slowmode of a channel")
    @commands.has_permissions(manage_channels=True)
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def slowmode(self, ctx, seconds: int):
        if seconds <= 21600 and seconds > 0:
            embed = discord.Embed(Title=f"Slowmode ", color=discord.Colour.random())

            embed.add_field(
                name=f"Slowmode Changed !",
                value=f"Slowmode is now set to {seconds} seconds in this channel",
            )
            embed.set_image(
