...
class Mod(commands.Cog):
    ...
    async def snipe(self, ctx):
        channel = ctx.channel
        try:
            embed = discord.Embed(name = f"Last deleted message in #{channel.name}", description = snipe_message_content[channel.id], color = discord.Colour.random())
    ...
    async def invite(self, ctx):
        embed = discord.Embed(title = 'Invite Me!', description = 'https://discord.com/api/oauth2/authorize?client_id=850392624154148905&permissions=8&scope=bot', colour = discord.Colour.random())
        embed.set_footer(icon_url = ctx.author.avatar_url, text = f"Requested by {ctx.author.name}, thanks for using me!")
        await ctx.send(embed=embed)
